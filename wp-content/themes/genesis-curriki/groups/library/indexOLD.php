...
<div class="wrap container_12">
                                <div class="tab-contents ui-tabs-panel ui-widget-content ui-corner-bottom" id="activity" aria-labelledby="ui-id-1" role="tabpanel" aria-hidden="false">
                                    <div class="activity-sidebar page-sidebar grid_2">
                                        ...
                                        ?>
                                        <h4 class="sidebar-title">Recently Active</h4>
                                        <div class="recently-active member-card card rounded-borders-full border-grey">
                                            ...
                                            <li class="member">
                                                ...
                                                <?php                                                        
                                                        ...
                                                        if(empty($userinfo->uniqueavatarfile)){
                                                            ...
                                                            echo '<img width="150" height="150" alt="Profile picture of '.$full_name.'" class="avatar user-'.$userinfo->uniqueavatarfile.'-avatar avatar-'.$userinfo->uniqueavatarfile.' photo" src="https://' . DBI_AWS_S3_CDN_URL . '//avatars/'.$userinfo->uniqueavatarfile.'">';
                                                        }                                                        
                                                ?>
                                            ...
                                            <a href="<?php echo $group_url; ?>members">
                                                <div class="card-button">Browse All Members</div>
                                            </a>
                                        </div>
                                        <?php } ?>
...
global $wpdb;
    ...
    if(!empty($me->uniqueavatarfile)) $myphoto = "https://" . DBI_AWS_S3_CDN_URL . "//avatars/".$me->uniqueavatarfile; else $myphoto = get_stylesheet_directory_uri() . '/images/user-icon-sample.png';
    ...
    foreach($resources as $collection){
        ...
        {
            $myphoto = "https://" . DBI_AWS_S3_CDN_URL . "//avatars/".$me->uniqueavatarfile;
        }else{
			...
			$library .= '<div class="library-asset rounded-borders-full border-grey library-collection">';
				...
				$library .= '<div class="library-title vertical-align"><a href="'.get_bloginfo('url').'/oer/?rid='.$collection->resourceid.'&back_url='.  base64_encode($group_url.'library').'">'.($collection->title?$collection->title:'Go to Collection').'</a></div>';
				$library .= '<div class="library-author vertical-align">';
					...
					$library .= '</div>';
					$library .= '<div class="member-more"><a href="'.get_bloginfo('url').'/user-library?userid='.$collection->contributorid.'">More from this member</a></div>';
				$library .= '</div>';
				$library .= '<div class="library-rating rating vertical-align"><span class="member-rating-title">Member Rating</span>';
					...
					$library .= '<a href="javascript:;" onclick="jQuery(\'#rate_resource-dialog\').show(); jQuery(\'#review-resource-id\').val('.$collection->resourceid.'); jQuery(\'.curriki-review-title\').html(\''.($collection->title?$collection->title:'Go to Collection').'\'); setInterval(function () {jQuery( \'#rate_resource-dialog\' ).center()}, 1);">Rate this resource</a>';
				$library .= '</div>';
