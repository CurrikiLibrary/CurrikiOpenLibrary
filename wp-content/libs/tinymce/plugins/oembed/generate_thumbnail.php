...
if(is_array($headers["Content-Type"])){
    if(count($headers["Content-Type"]) > 0){
        $response['ext'] = strtolower(current(explode(';', end(explode('/', $headers["Content-Type"][0])))));
    }
    else{
        $response['ext'] ='';
    }
...
if ($response['ext'] == 'html') {
  $response['ext'] = 'jpg';
  $command = "/usr/local/bin/wkhtmltoimage --disable-smart-width --width {$width} --height {$height} --crop-y 0 --crop-h {$height} --load-error-handling ignore";
  ...
  $response['url'] = 'https://' . DBI_AWS_S3_CDN_URL . '/resourceimgs/561d08936193b.png';
}
